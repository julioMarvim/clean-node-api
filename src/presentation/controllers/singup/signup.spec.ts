...
const makeSut = (): SutTypes => {
  ...
  const sut = new SignUpController(emailValidatorStub, addAccounStub)
...
describe('SignUp Controller', () => {
  ...
  test('Shoud return 400 if no passwordConfirmation fails', () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Shoud call AddAccount with correct values', () => {
    ...
    const addSoy = jest.spyOn(addAccounStub, 'add')
    const httpRequest = {
      body: {
        name: 'any_name',
        email: 'any_email@mail.com',
    ...
    expect(addSoy).toHaveBeenCalledWith({
      name: 'any_name',
      email: 'any_email@mail.com',
      password: 'any_password'
    })
  ...
  test('Shoud return 500 if AddAccount throws', () => {
    const { sut, addAccounStub } = makeSut()
    jest.spyOn(addAccounStub, 'add').mockImplementationOnce(() => {
      throw new Error()
    })
  ...
  test('Shoud return 200 if an valid data is provided', () => {
    const { sut } = makeSut()
